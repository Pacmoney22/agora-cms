...
MinIO auto-initializes with two buckets: `nextgen-media` (public download) and `nextgen-labels` (private).
...
pnpm --filter @nextgen-cms/database db:generate
...
pnpm --filter @nextgen-cms/content-service dev
pnpm --filter @nextgen-cms/commerce-service dev
pnpm --filter @nextgen-cms/page-builder dev
```
...
| super@nextgen-cms.dev | super_admin |
...
**Package:** `@nextgen-cms/content-service`
...
**Package:** `@nextgen-cms/commerce-service`
...
**Package:** `@nextgen-cms/integration-service`
**Bootstrap:** `services/integration-service/src/main.ts`
**Swagger:** http://localhost:3003/api/docs

The integration service connects NextGen CMS to external platforms. It implements the adapter pattern with feature-flag-based activation (`STRIPE_ENABLED`, `GA4_ENABLED`, `SALESFORCE_ENABLED`).
...
All integrations implement typed interfaces from `@nextgen-cms/shared`:
...
**Package:** `@nextgen-cms/shipping-gateway`
...
Carriers implement the `ICarrierAdapter` interface from `@nextgen-cms/shared`:
...
**Package:** `@nextgen-cms/course-service`
...
**Package:** `@nextgen-cms/page-builder`
...
**Package:** `@nextgen-cms/storefront`
...
**Package:** `@nextgen-cms/admin-dashboard`
...
```typescript
import { useSharedProps, type SharedProps } from '@nextgen-cms/ui';
...
The `hasMinimumRole(userRole, requiredRole)` function from `@nextgen-cms/shared` checks if a user's role meets the minimum required level.
...
NextGen CMS uses MinIO (S3-compatible) for all file storage. In production, this can be swapped for AWS S3 or any S3-compatible service.

#### Buckets

| Bucket | Purpose | Access |
|---|---|---|
| `nextgen-media` | Media uploads (images, videos, documents) | Public download |
| `nextgen-labels` | Shipping labels (PDFs) | Private |
...
pnpm --filter @nextgen-cms/database db:migrate
...
pnpm --filter @nextgen-cms/database db:migrate:deploy
```
...
pnpm --filter @nextgen-cms/database db:push
...
```typescript
import { MyWidget } from '@nextgen-cms/ui';
import myWidgetSchema from '@nextgen-cms/ui/src/schemas/my-widget.schema.json';
...
|---|---|---|
| `DATABASE_URL` | `postgresql://nextgen:nextgen_dev@localhost:5432/nextgen_cms` | PostgreSQL connection string |
...
| `S3_BUCKET_MEDIA` | `nextgen-media` | Media upload bucket |
| `S3_BUCKET_LABELS` | `nextgen-labels` | Shipping label bucket |
...
```bash
docker exec nextgen-minio mc alias set local http://localhost:9000 minioadmin minioadmin
docker exec nextgen-minio mc mb --ignore-existing local/nextgen-media
docker exec nextgen-minio mc mb --ignore-existing local/nextgen-labels
docker exec nextgen-minio mc anonymous set download local/nextgen-media
```

### Database

**Problem:** Prisma Client not generated
**Solution:** Run `pnpm --filter @nextgen-cms/database db:generate` before starting services.
...
**Solution:** After editing `schema.prisma`, run `pnpm --filter @nextgen-cms/database db:generate` to validate.
...
**Problem:** `@nextgen-cms/ui` components not found
**Solution:** Ensure `transpilePackages` includes `@nextgen-cms/ui` in `next.config.js` (already configured for storefront and page-builder).

**Problem:** TypeScript errors after schema changes
**Solution:** Rebuild shared packages: `pnpm --filter @nextgen-cms/shared build && pnpm --filter @nextgen-cms/database db:generate`
...
- **Linting**: ESLint with `@nextgen-cms/eslint-config`
...
2. Use `@nextgen-cms/tsconfig/nestjs.json` as TypeScript base
3. Use `@nextgen-cms/eslint-config` for linting
4. Add `workspace:*` dependencies on `@nextgen-cms/database` and `@nextgen-cms/shared`
...
3. Use `@nextgen-cms/tsconfig/nextjs.json` as TypeScript base
