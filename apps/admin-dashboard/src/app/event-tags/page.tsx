...
export default function EventTagsPage() {
  ...
  const { data: registry, isLoading } = useQuery({
    ...
    queryFn: () => settingsApi.get('event_tags').catch((): TagRegistry => ({ tags: [] })),
  });

  const tags: EventTag[] = (registry as TagRegistry)?.tags || [];
  ...
  const saveMutation = useMutation({
    mutationFn: (updated: EventTag[]) =>
      settingsApi.update('event_tags', { tags: updated } as any),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['settings', 'event_tags'] });
    ...
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage tags for organizing and filtering events</p>
        </div>
        <button
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mb-6 rounded-lg bg-white p-5 shadow">
          ...
          <div className="mt-4">
            <MediaPicker label="Tag Image" value={form.image} onChange={(v) => setForm({ ...form, image: v })} accept="image/*" helpText="Displayed on the tag archive page" />
          </div>
      ...
      {tags.length > 5 && (
        <div className="mb-4">
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} className="w-64 rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" placeholder="Search tags..." />
        </div>
      )}
      ...
      {isLoading ? (
                ...
                <tr key={tag.id} className="border-b border-gray-50 hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <span className="inline-block h-3 w-3 rounded-full shrink-0" style={{ backgroundColor: tag.color || '#6b7280' }} />
                  ...
                  <td className="px-4 py-3">
                    <span className="text-xs text-gray-500 truncate max-w-xs block">{tag.description || '\u2014'}</span>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      <button onClick={() => startEdit(tag)} className="text-xs text-blue-600 hover:text-blue-800">Edit</button>
                      <button onClick={() => deleteTag(tag.id, tag.name)} className="text-xs text-red-500 hover:text-red-700">Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="rounded-lg border-2 border-dashed border-gray-300 py-16 text-center">
          <p className="text-sm text-gray-400">No event tags yet.</p>
