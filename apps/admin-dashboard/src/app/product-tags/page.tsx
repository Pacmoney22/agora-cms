...
export default function ProductTagsPage() {
  ...
  const { data: registry, isLoading } = useQuery({
    ...
    queryFn: () => settingsApi.get('product_tags').catch((): TagRegistry => ({ tags: [] })),
  });

  const tags: ProductTag[] = (registry as TagRegistry)?.tags || [];
  ...
  const saveMutation = useMutation({
    mutationFn: (updated: ProductTag[]) =>
      settingsApi.update('product_tags', { tags: updated } as any),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['settings', 'product_tags'] });
    ...
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage tags for organizing and filtering products</p>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border-2 border-dashed border-gray-300 py-16 text-center">
          <p className="text-sm text-gray-400">No product tags yet.</p>
          <button
            onClick={() => { resetForm(); setShowForm(true); }}
