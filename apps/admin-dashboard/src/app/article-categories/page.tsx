...
function buildTree(categories: ArticleCategory[], parentId: string = ''): (ArticleCategory & { children: ArticleCategory[] })[] {
  return categories
...
export default function ArticleCategoriesPage() {
  ...
  const { data: registry, isLoading } = useQuery({
    ...
    queryFn: () => settingsApi.get('article_categories').catch((): CategoryRegistry => ({
      categories: [
        { id: '1', name: 'General', slug: 'general', description: '', parentId: '', image: '', position: 0, seo: { title: '', description: '', keywords: '', ogImage: '' } },
        { id: '2', name: 'News', slug: 'news', description: '', parentId: '', image: '', position: 1, seo: { title: '', description: '', keywords: '', ogImage: '' } },
        { id: '3', name: 'Tutorial', slug: 'tutorial', description: '', parentId: '', image: '', position: 2, seo: { title: '', description: '', keywords: '', ogImage: '' } },
        { id: '4', name: 'Announcement', slug: 'announcement', description: '', parentId: '', image: '', position: 3, seo: { title: '', description: '', keywords: '', ogImage: '' } },
      ],
    })),
  });

  const categories: ArticleCategory[] = (registry as CategoryRegistry)?.categories || [];
  const tree = buildTree(categories);

  const saveMutation = useMutation({
    mutationFn: (updated: ArticleCategory[]) =>
      settingsApi.update('article_categories', { categories: updated } as any),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['settings', 'article_categories'] });
    ...
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Organize blog articles into a category hierarchy</p>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border-2 border-dashed border-gray-300 py-16 text-center">
          <p className="text-sm text-gray-400">No article categories yet.</p>
          <button
            onClick={() => { resetForm(); setShowForm(true); }}
...
function CategoryTree({
  ...
  nodes: (ArticleCategory & { children: ArticleCategory[] })[];
