...
export default function ArticleTagsPage() {
  ...
  const { data: registry, isLoading } = useQuery({
    ...
    queryFn: () => settingsApi.get('article_tags').catch((): TagRegistry => ({ tags: [] })),
  });

  const tags: ArticleTag[] = (registry as TagRegistry)?.tags || [];
  ...
  const saveMutation = useMutation({
    mutationFn: (updated: ArticleTag[]) =>
      settingsApi.update('article_tags', { tags: updated } as any),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['settings', 'article_tags'] });
  ...
  const startEdit = (tag: ArticleTag) => {
    setForm({
      name: tag.name,
      slug: tag.slug,
  ...
  return (
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Article Tags</h1>
          <p className="mt-1 text-sm text-gray-500">Manage tags for organizing and filtering blog articles</p>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border-2 border-dashed border-gray-300 py-16 text-center">
          <p className="text-sm text-gray-400">No article tags yet.</p>
          <button
            onClick={() => { resetForm(); setShowForm(true); }}
